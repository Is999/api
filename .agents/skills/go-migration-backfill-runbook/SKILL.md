...
- 仓库只维护完整初始化 DDL/DML，直接修改原始资产，不为需求增量新建迁移 SQL；这是资产治理规则，不是只允许空库执行的限制。
...
- 本地 SQL 不提交、不暂存、不登记到项目迁移表；执行证据保存在工单或 DBA 平台。
- 运行时代码真实调用的 `*.sql.tmpl` 仍是代码资产；不要把它与一次性 DBA SQL 混淆。
- 已产品化的 CLI、分表或部署工具可维护稳定 SQL 资产，但必须有真实调用方、明确输入、测试和长期复用边界；不能借此提交单次需求执行包。

## 迁移执行与服务启动

- Admin 与 API 分别使用 `admin_schema_migrations`、`api_schema_migrations`，同库执行使用共同迁移锁；不得混用登记归属或绕过执行互斥。
- 命令可在非空库执行未登记的幂等资产并续跑失败资产；已登记且元数据匹配的项跳过。执行前仍核对 bootstrap/destructive 授权、业务行保护和备份，不能因 SQL 可重跑就取消安全校验。
- 命令对本仓版本、名称、资产或 checksum 冲突仍报错，不能伪造登记或删除记录强制重放；直接命令行执行 SQL 也要先确认目标、幂等性、数据影响与授权。
- DBA 人工变更后的登记版本差异不作为服务启动门禁；应用继续校验真实配置、连接与必需能力，不能把登记一致当作业务结构完整的证明。

## 执行流程

1. 确认目标环境、实例、库表、真实结构与数据量、Model、索引、读写路径、API/任务调用方，以及已授权的执行、发布和恢复边界。
...
5. 存量增量变更根据真实库状态生成本地 SQL；明确执行初始化时，先验证资产幂等性、业务行保护和权限，再使用现有迁移命令，不能把库非空作为拒绝条件，也不能把完整初始化当作任意存量升级脚本。
...
- 删除字段、索引或表前，证明所有当前调用方已停止依赖并确认发布与恢复顺序；不预设历史兼容窗口，更不能通过保留旧协议分支代替数据与调用方核验。
...
- 按当前调用方和真实库状态确定 SQL、代码切换、回填与清理顺序，逐阶段写明验证与停止条件；不默认生成兼容代码或双读双写分支。
