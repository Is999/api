...
- 内容要求：先写消费组件和用途，再用简短语句说明源码已定义的数据形状、取值/单位、缺省/空值/零值、normalize/getter、reload/restart、敏感信息及跨字段约束；一个字段紧贴自身说明，一句话能说清楚不写成长段，也不省略真实边界。
...
- struct/tag/default/normalize/getter 变化：覆盖字段缺失、显式零值、合法边界、越界、当前配置样例，以及未知或已移除字段的拒绝行为，不为旧字段增加别名或回退分支。
