...
1. 先检查失败命令的环境、库/服务目标和写入副作用，确认隔离与授权后复现并记录第一处真实错误；原命令可能操作真实数据时先报告风险，不能因“复现”而直接重跑。
2. 分类失败：编译、vet、单测逻辑、集成依赖、race、timeout、flaky 顺序、缓存、fixture 或环境配置。
3. 区分本轮引入的问题与仓库已有健康噪声，不能用无关失败掩盖新回归。
4. 使用真实 package path、`-run` 和 `-count=1` 缩小范围；`-run '^$'` 仍执行包初始化与 `TestMain`，不能当作无副作用的仅编译检查。只验证编译时使用 `go test -c -o <临时目录>/<包名>.test <单个包路径>`，不要运行产物。
5. 用户已授权修复时，从生产源码、测试、fixture 或配置修正根因；只分析、解释或复核任务保持只读并交付证据。不得为通过检查跳过、删除或削弱有效测试。
